...
const addInputBtn1 = call(".add1");
const calculateBtn = call(".calculate");
const calculateBtn1 = call(".calculate1");
...
let closeBtn1 = document.querySelector(".cancelBtn1");
let gpaBtn = document.querySelector(".GPA");
let overallgpa = document.getElementById("overall-gpa");
let gpaxBtn = document.querySelector(".GPAX");
let overallgpax = document.getElementById("overall-gpax");
...
contactBtn.addEventListener("click", () => (modal1.style.display = "block"));
closeBtn.addEventListener("click", () => (modal.style.display = "none"));
closeBtn1.addEventListener("click", () => (modal1.style.display = "none"));
...
function stopCalculation() {
  let firstClass = averageGPA(years.value, 3.62);
  let content = `
    <div class="display__result__content">
      <h3>${firstUpper(userName.value)} you're on a ${
    years.value
  } years program</h3>
      <p><strong>${cgpaCal(cgpaArray)}</strong> is your current GPAX score</p>
      <small><em>Note: The average score is approximated</em></small>
      <div>
        ${
          firstClass > 4.00
            ? ""
            : `<p>You'll need <strong>${firstClass}</strong> to remain at <strong>1st class honor</strong> for each of the semesters left</p>`
        }
        <p>You'll need <strong>${averageGPA(
          years.value,
          3.27
        )}</strong> to remain at with a <strong>2nd class honor</strong> for each of the semesters left</p>
      </div>
  ...
  function addRow(num) {
    let str = `
      <div class="display__output__container">
        <input type="text" name="courseCode" class="courseCode" placeholder="Subject" />
